...
def downloadsocks(choice):
    ...
    if choice == "4":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/nangsontay2005/fuckproxyvn/main/proxy.txt?token=APMGZZRCC6EEVO7PMRA7ISDBG3FF4", timeout=5)                             
            f.write(r.content)
        except:
            pass
        try:
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://api.proxyscrape.com/?request=displayproxies&proxytype=socks5&country=vn",
                             timeout=5)
            f.write(r.content)
        except:
            pass
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://api.proxyscrape.com/?request=displayproxies&proxytype=http&country=vn",
                             timeout=5)
            f.write(r.content)
        except:
            pass
